...
citizen.get("/api/citizen/listartodos", (req, res) => {
  let query ="SELECT idciudadano, roles_idrol, especies_idespecie ,nombre_ciudadano, apellido_ciudadano, email_ciudadano, apodo_ciudadano, fechaorigen, nombre_especie, nombre_rol FROM ciudadanos inner join especies on idespecie = especies_idespecie inner join roles on idrol = roles_idrol order by idciudadano asc";
...
citizen.get("/api/citizen/listarid/:id", (req, res) => {
  ...
  let query ="SELECT idciudadano, roles_idrol, especies_idespecie ,nombre_ciudadano, apellido_ciudadano, email_ciudadano, apodo_ciudadano, fechaorigen, password_ciudadano, nombre_especie, nombre_rol FROM ciudadanos inner join especies on idespecie = especies_idespecie inner join roles on idrol = roles_idrol WHERE idciudadano = ?";
